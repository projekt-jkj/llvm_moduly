...
case "$TOOLSET" in
	...
	clang)
		...
		LLVM_OPTIONS+=(-DLLVM_ENABLE_LIBCXX=ON -DLLVM_STATIC_LINK_CXX_STDLIB=ON -DLLVM_ENABLE_LLD=ON);
		;;
	clang_dynamic)
		CMAKE_OPTIONS+=(-DCMAKE_C_COMPILER=clang -DCMAKE_CXX_COMPILER=clang++);
		LLVM_OPTIONS+=(-DLLVM_ENABLE_LIBCXX=ON -DLLVM_STATIC_LINK_CXX_STDLIB=OFF -DLLVM_ENABLE_LLD=ON);
		;;
	clang_gcc)
		CMAKE_OPTIONS+=(-DCMAKE_C_COMPILER=clang -DCMAKE_CXX_COMPILER=clang++);
		LLVM_OPTIONS+=(-DLLVM_ENABLE_LIBCXX=OFF -DLLVM_STATIC_LINK_CXX_STDLIB=OFF -DLLVM_ENABLE_LLD=ON);
		;;
esac
